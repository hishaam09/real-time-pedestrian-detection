...
KNOWN_DISTANCE = 1.0  # meter (you can measure with a ruler)
KNOWN_WIDTH = 0.5     # width of known object (e.g., your shoulder width or A4 = 0.21m)
...
while True:
    ...
    for pred in predictions:
        # scores slice (after 4 coords) - this depends on your ONNX variant but this matches your working code
        scores = pred[4:]
    ...
    for i in indices:
        ...
        # measure distance using box width (you used width earlier)
    ...
    # bottom title and time (centered and right)
    title_text = "AI Pedestrian Detection - Low Light"
    title_x = max(10, (w // 2) - (len(title_text) * 6))  # rough centering
    cv2.putText(frame, title_text, (title_x, h - 35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,255), 2)

    # mode & timestamp on bottom left / right
